...
class Program
{
    ...
    static string monitoredFolder = @"C:\Users\victo\Desktop\Lucrarea Nr.3\File";
    static string logFilePath = @"C:\Users\victo\Desktop\Lucrarea Nr.3\log.txt";
    static string commitFoldar = @"C:\Users\victo\Desktop\Lucrarea Nr.3\Snapshotul";
                ...
                case "info":
                    if (commandParts.Length > 1)
                    {
                        if (commandParts[1].ToLower() == "commit")
                        {
                            ...
                            string? snapshotFileName = Console.ReadLine();
                            string snapshotFilePath = Path.Combine(@"C:\Users\victo\Desktop\Lucrarea Nr.3\Snapshotul", snapshotFileName);
    ...
    static void Commit()
    {
        ...
        string commitFolder = Path.Combine(@"C:\Users\victo\Desktop\Lucrarea Nr.3", "Snapshotul");
    ...
    static void Info(string fileName)
    {
        ...
        if (File.Exists(filePath))
        {
          ...
          fileInfo.Extension.Equals(".java", StringComparison.OrdinalIgnoreCase))
            {
                int numLines = File.ReadAllLines(filePath).Length;
                Console.WriteLine($"Numar de linii: {numLines}");

                // Numărul de clase și metode pentru fișierele .java și .py
                int numClasses = 0;
                int numMethods = 0;
                ...
                using (StreamReader sr = new StreamReader(filePath))
                {
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        else if (insideClass && (line.StartsWith("public ") || line.StartsWith("private ") || line.StartsWith("protected ")))
                        {
                        ...
                        else if (line.StartsWith("}") && insideClass)
                        {
                            insideClass = false;
                        }
                    }
                }
                Console.WriteLine($"Numar de clase: {numClasses}");
                Console.WriteLine($"Numar de metode: {numMethods}");
            }
        }
        else
        {
